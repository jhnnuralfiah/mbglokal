    ...
    try {
        ...
        if (data.success) {
            ...
            setTimeout(() => {
                ...
                else if (data.role === "PETANI") window.location.href = "petani-dashboard.html";
                else if (data.role === "SEKOLAH") window.location.href = "sekolah-dashboard.html";
            }, 1200);
...
async function tambahKomoditas() {
    ...
    try {
        const res = await fetch(`${BASE_URL}/komoditas`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ namaBahan: nama, stokSaatIni: parseFloat(stok), satuan })
        });
        ...
        data.forEach((item, i) => {
            body.innerHTML += `
                <tr>
                    ...
                    <td>
                        <div class="action-btn">
                            <button class="delete-btn" onclick="hapusDetailMenu(${item.idDetail})">🗑️ Hapus</button>
                        </div>
                    </td>
                </tr>
            `;
...
async function hapusDetailMenu(id) {
    ...
    try {
        ...
        showToast("Gagal hapus detail menu", "error");
    }
}
...
async function loadDistribusi() {
    ...
    if (role === "SEKOLAH") url = `${BASE_URL}/distribusi/user/${idUser}`;
    ...
    try {
        ...
        data.forEach((item, i) => {
            ...
            if (item.status === "Proses") {
                statusBadge = `<span class="status-proses">Proses</span>`;
            } else if (item.status === "Dikirim") {
                statusBadge = `<span class="status-kirim">Dikirim</span>`;
            } else if (item.status === "Selesai") {
                statusBadge = `<span class="status-selesai">Selesai</span>`;
            }
            ...
            if (role === "SEKOLAH") {
                ...
                body.innerHTML += `
                    <tr>
                        ...
                        <td>
                            <div class="action-btn">
                                <button class="edit-btn" onclick="openEditDistribusi(${item.idDistribusi}, '${item.status}')">✏️ Edit</button>
                                <button class="delete-btn" onclick="hapusDistribusi(${item.idDistribusi})">🗑️ Hapus</button>
                            </div>
                        </td>
                    </tr>
                `;
        ...
        const total = document.getElementById("totalDistribusi");
        if (total) total.innerText = data.length;
...
async function loadDropdownPaket() {
    ...
    try {
        ...
        data.forEach(item => {
            select.innerHTML += `<option value="${item.idMenu}">${item.namaMenu}</option>`;
        });

    } catch { console.log("Gagal load dropdown paket"); }
}
...
async function loadDropdownPenerima() {
    ...
    try {
        const res = await fetch(`${BASE_URL}/penerima-manfaat`);
        const data = await res.json();

        select.innerHTML = `<option value="">Pilih Penerima</option>`;
        data.forEach(item => {
            if (item.role === "SEKOLAH") {
                select.innerHTML += `<option value="${item.idUser}">${item.namaInstansi}</option>`;
            }
        });

    } catch { console.log("Gagal load dropdown penerima"); }
}
...
async function loadDropdownDetailMenu() {
    ...
    try {
        ...
        komoditasData.forEach(item => {
            komoditasSelect.innerHTML += `<option value="${item.idKomoditas}">${item.namaBahan}</option>`;
        });

    } catch { console.log("Gagal load dropdown detail menu"); }
}
...
async function loadDashboardPreview() {
    ...
    } catch { console.log("Gagal load dashboard preview"); }
}
...
async function loadDashboardSekolah() {
    ...
    try {
        ...
        if (totalEl) totalEl.innerText = mine.length;
        if (prosesEl) prosesEl.innerText = mine.filter(d => d.status === "Proses").length;
        if (selesaiEl) selesaiEl.innerText = mine.filter(d => d.status === "Selesai").length;
